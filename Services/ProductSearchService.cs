    ...
    : IProductSearchService
{
        ...
        int? categoryId = null)
    {
        ...
        // Search now returns the full ranked list; callers may still pass this value.
        _ = maxResults;
        ...
        const double minSearchScore = 0.05;

        var scored = products
        .Select(p =>
        {
            var score = ComputeAutocompleteScore(normalizedQuery, p.Name.ToLowerInvariant());
            return (Product: p, Score: score);
        })
        .Where(x => x.Score >= minSearchScore)
        .OrderByDescending(x => x.Score)
        .ThenBy(x => x.Product.Name)
        .ToList();

        logger.LogDebug("Autocomplete-style search for '{Query}' returned {Count} results", query, scored.Count);
